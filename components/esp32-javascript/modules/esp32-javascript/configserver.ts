...
export var requestHandler: ((req: Esp32JsRequest, res: Esp32JsResponse) => void)[] = [];
export var baExceptionPathes: string[] = [];

export function getHeader(statusCode: number, additionalHeaders: string) {
  return [
    "HTTP/1.1 ",
    statusCode,
    " OK\r\n", //
...
export function redirect(res: Esp32JsResponse, location: string) {
...
function page(
  ...
  res.write(
    '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/pure/1.0.1/pure-min.css"></head>'
  );
  res.write(
    '<body><div class="pure-g"><div class="pure-u-1"><div class="l-box"><h1>'
  );
  res.write(headline);
  res.write("</h1>");
...
export function startConfigServer() {
  ...
  var authString =
    "Basic " +
    btoa(
      defaultConfig.basicAuthUsername + ":" + defaultConfig.basicAuthPassword
    );
  httpServer(80, false, function (req, res) {
    if (
      ...
      res.write(
        getHeader(401, 'WWW-Authenticate: Basic realm="Enter credentials"\r\n')
      );
      ...
      if (req.method === "GET") {
        ...
        el_store(
          "config.offline",
          config.offline === "true" ? "true" : "false"
        );
