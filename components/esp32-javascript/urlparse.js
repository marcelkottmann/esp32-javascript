	...
	var anchorElement = {
		...
		resolve: function (rel) {
			...
			if (rel.substr(0, 1) === '/') {
				...
				if (!(!directoryPath || directoryPath.substr(directoryPath.length - 1, 1) === '/')) {
	...
	Object.defineProperties(anchorElement,
		{
			'href': {
				'get': function () {
					var href = anchorElement.origin + anchorElement.pathname + anchorElement.search + anchorElement.hash;
					return href;
				},
				'set': function (value) {
			...
			{
				'get': function () {
					return anchorElement.hostname + (anchorElement.port ? ':' + anchorElement.port : '');
				},
				'set': function (value) {
